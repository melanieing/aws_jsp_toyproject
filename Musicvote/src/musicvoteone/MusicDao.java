...
public class MusicDao {
	...
	// 2번 음악장르 등록하기
	public boolean insertMusic(String input) {
		boolean ret = false;
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public List<MusicDto> selectAll() {
		...
		List<MusicDto> ls = new ArrayList<>();
		...
		try {
			...
			while (rs.next()) {
				MusicDto tmp = new MusicDto(rs.getLong(1), rs.getString(2), rs.getLong(3));
				ls.add(tmp);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	// 표 누적
	public boolean updatevote(String number) {
	...
	// 4번 설문항목 변경하기
	public void altermusic(String input, String newInput) {

		Connection conn = null;
		PreparedStatement pstmt = null;

		String sql = "update \"MUSIC_VOTE\" set \"MUSIC_TYPE\"=? where \"MUSIC_TYPE\"=?";
		try {
			conn = ConnUtil.getConnection();
